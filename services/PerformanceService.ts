...
class PerformanceService {
  ...
  private async getMemoryInfo(): Promise<MemoryInfo | null> {
    try {
      ...
      const estimatedMemory = {
        usedJSHeapSize:
          Math.floor(Math.random() * 100 * 1024 * 1024) + 50 * 1024 * 1024, // 50-150MB
  ...
  measureRenderTime(componentName: string, renderTime: number): void {
    ...
    if (this.performanceMetrics.length > 0) {
      this.performanceMetrics[this.performanceMetrics.length - 1].renderTime =
        renderTime;
    }
  }
  ...
  getPerformanceStats(): {
    ...
    const totalCacheAccesses = Array.from(this.memoryCache.values()).reduce(
      (sum, entry) => sum + entry.accessCount,
      0,
    );
    const cacheHitRate =
      this.memoryCache.size > 0
        ? (totalCacheAccesses - this.memoryCache.size) / totalCacheAccesses
        : 0;
