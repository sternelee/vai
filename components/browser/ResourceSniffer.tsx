...
export default function ResourceSniffer({
  ...
  useEffect(() => {
    if (selectedFilter === 'all') {
      ...
      setFilteredResources(resources.filter(item => item.type === selectedFilter));
    }
    ...
    if (resource.type === 'image' || resource.type === 'video') {
      ...
      Alert.alert(
        ...
        [
          ...
          { text: '在浏览器中打开', onPress: () => handleOpenInBrowser(resource.url) },
        ]
      );
    }
  };
  ...
  const renderFilterButton = (filter: typeof filters[0]) => (
    ...
    >
      <Ionicons
        ...
        size={16}
        color={selectedFilter === filter.key ? '#FFFFFF' : (isDark ? '#FFFFFF' : '#000000')}
      />
      <Text
        style={[
  ...
  const renderPreviewModal = () => (
    ...
    >
      <View style={[styles.previewContainer, { backgroundColor: isDark ? '#1C1C1E' : '#FFFFFF' }]}>
        <View style={styles.previewHeader}>
          <Text style={[styles.previewTitle, { color: isDark ? '#FFFFFF' : '#000000' }]}>
    ...
    >
      <View style={[styles.container, { backgroundColor: isDark ? '#1C1C1E' : '#FFFFFF' }]}>
        ...
        <View style={[styles.header, { borderBottomColor: isDark ? '#2C2C2E' : '#E5E5EA' }]}>
          ...
          <Text style={[styles.pageInfo, { color: isDark ? '#8E8E93' : '#6B6B6B' }]}>
            {currentPageTitle}
          </Text>
        </View>
        ...
        <View style={styles.countContainer}>
          <Text style={[styles.countText, { color: isDark ? '#8E8E93' : '#6B6B6B' }]}>
        ...
        {isLoading ? (
          <View style={styles.loadingContainer}>
            <Text style={[styles.loadingText, { color: isDark ? '#8E8E93' : '#6B6B6B' }]}>
              正在扫描页面资源...
            </Text>
          </View>
        ) : (
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                ...
                <Text style={[styles.emptySubtext, { color: isDark ? '#8E8E93' : '#6B6B6B' }]}>
                  点击刷新按钮重新扫描
                </Text>
              </View>
            }
