...
export default function MCPManager({ visible, onClose }: MCPManagerProps) {
  ...
  const [selectedTab, setSelectedTab] = useState<'servers' | 'tools' | 'add'>('servers');
  ...
  const renderServerCard = (server: MCPServer) => (
    <View key={server.id} style={[styles.serverCard, isDark && styles.serverCardDark]}>
      <View style={styles.serverHeader}>
        <View style={styles.serverInfo}>
          <View style={styles.serverTitleRow}>
            <Text style={[styles.serverName, isDark && styles.textDark]}>{server.name}</Text>
            <View style={[styles.statusBadge, { backgroundColor: getStatusColor(server.status) }]}>
              <Text style={styles.statusText}>{getStatusText(server.status)}</Text>
            </View>
          </View>
          <Text style={[styles.serverDescription, isDark && styles.textSecondaryDark]}>
            {server.description}
          </Text>
          <Text style={[styles.serverUrl, isDark && styles.textSecondaryDark]} numberOfLines={1}>
            {server.url}
          </Text>
        </View>
          ...
          {server.tools.map(tool => (
            <View key={tool.name} style={styles.toolItem}>
              <View style={styles.toolInfo}>
                <Text style={[styles.toolName, isDark && styles.textDark]}>{tool.name}</Text>
                <Text style={[styles.toolDescription, isDark && styles.textSecondaryDark]} numberOfLines={1}>
                  {tool.description}
                </Text>
              </View>
            </View>
      ...
      {server.lastConnected && (
        <Text style={[styles.lastConnected, isDark && styles.textSecondaryDark]}>
    ...
    <ScrollView style={styles.tabContent} showsVerticalScrollIndicator={false}>
      {statistics && (
        <View style={[styles.statsCard, isDark && styles.statsCardDark]}>
          <Text style={[styles.statsTitle, isDark && styles.textDark]}>📊 Statistics</Text>
          <View style={styles.statsRow}>
            <View style={styles.statItem}>
              <Text style={[styles.statValue, isDark && styles.textDark]}>{statistics.totalServers}</Text>
              <Text style={[styles.statLabel, isDark && styles.textSecondaryDark]}>Servers</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={[styles.statValue, isDark && styles.textDark]}>{statistics.connectedServers}</Text>
              <Text style={[styles.statLabel, isDark && styles.textSecondaryDark]}>Connected</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={[styles.statValue, isDark && styles.textDark]}>{statistics.totalTools}</Text>
              <Text style={[styles.statLabel, isDark && styles.textSecondaryDark]}>Total Tools</Text>
            </View>
            <View style={styles.statItem}>
              <Text style={[styles.statValue, isDark && styles.textDark]}>{statistics.enabledTools}</Text>
              <Text style={[styles.statLabel, isDark && styles.textSecondaryDark]}>Enabled</Text>
            </View>
          </View>
        </View>
      )}
      ...
      {loading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.loadingText, isDark && styles.textDark]}>Loading servers...</Text>
        </View>
      ) : servers.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyIcon}>🔧</Text>
          <Text style={[styles.emptyText, isDark && styles.textDark]}>No MCP servers configured</Text>
    ...
    return (
      <ScrollView style={styles.tabContent} showsVerticalScrollIndicator={false}>
        {enabledToolGroups.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyIcon}>🔧</Text>
            <Text style={[styles.emptyText, isDark && styles.textDark]}>No tools available</Text>
            <Text style={[styles.emptySubtext, isDark && styles.textSecondaryDark]}>
          ...
          enabledToolGroups.map(group => {
            ...
            return (
              <View key={group.serverId} style={[styles.toolGroupCard, isDark && styles.toolGroupCardDark]}>
                <Text style={[styles.toolGroupTitle, isDark && styles.textDark]}>
                  {server?.name || group.serverId}
                </Text>
                <Text style={[styles.toolGroupSubtitle, isDark && styles.textSecondaryDark]}>
  ...
  const renderAddTab = () => (
    <ScrollView style={styles.tabContent} showsVerticalScrollIndicator={false}>
      <View style={[styles.formCard, isDark && styles.formCardDark]}>
        <Text style={[styles.formTitle, isDark && styles.textDark]}>Add MCP Server</Text>
        
        <View style={styles.formField}>
          <Text style={[styles.fieldLabel, isDark && styles.textDark]}>Name *</Text>
          <TextInput
        ...
        <View style={styles.formField}>
          <Text style={[styles.fieldLabel, isDark && styles.textDark]}>Description</Text>
          <TextInput
            style={[styles.fieldInput, isDark && styles.fieldInputDark]}
            value={newServer.description}
            onChangeText={(text) => setNewServer(prev => ({ ...prev, description: text }))}
        ...
        <View style={styles.formField}>
          <Text style={[styles.fieldLabel, isDark && styles.textDark]}>URL *</Text>
          <TextInput
            style={[styles.fieldInput, isDark && styles.fieldInputDark]}
            value={newServer.url}
        ...
        <View style={styles.formField}>
          <Text style={[styles.fieldLabel, isDark && styles.textDark]}>Headers (Optional)</Text>
          
          {newServer.headers && Object.entries(newServer.headers).map(([key, value]) => (
            <View key={key} style={styles.headerItem}>
              <Text style={[styles.headerText, isDark && styles.textDark]}>{key}: {value}</Text>
          ...
          <View style={styles.headerInputRow}>
            ...
            <TextInput
              ...
              placeholderTextColor={isDark ? '#8E8E93' : '#999'}
            />
            <TouchableOpacity style={styles.addHeaderButton} onPress={handleAddHeader}>
        ...
        <View style={styles.formField}>
          <View style={styles.switchRow}>
            <Text style={[styles.fieldLabel, isDark && styles.textDark]}>Enable automatically</Text>
            <Switch
              value={newServer.enabled || false}
              onValueChange={(value) => setNewServer(prev => ({ ...prev, enabled: value }))}
              trackColor={{ false: '#767577', true: '#007AFF' }}
        ...
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity
            style={[styles.addButton, (!newServer.name || !newServer.url) && styles.buttonDisabled]}
      ...
      <View style={[styles.infoCard, isDark && styles.infoCardDark]}>
        <Text style={[styles.infoTitle, isDark && styles.textDark]}>ℹ️ About MCP</Text>
        <Text style={[styles.infoText, isDark && styles.textSecondaryDark]}>
          Model Context Protocol (MCP) allows AI to access external tools and services through standardized interfaces.
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={[styles.container, isDark && styles.containerDark]}>
        ...
        <View style={[styles.header, isDark && styles.headerDark]}>
          <Text style={[styles.title, isDark && styles.textDark]}>🔧 MCP Tools</Text>
