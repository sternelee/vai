...
export default function HomePage({
  ...
  const renderQuickAction = (action: typeof quickActions[0]) => (
    ...
    >
      ...
      <Text style={[styles.quickActionTitle, { color: isDark ? '#FFFFFF' : '#000000' }]}>
        {action.title}
      </Text>
    </TouchableOpacity>
  );

  const renderShortcut = (shortcut: HomePageShortcut | typeof recommendedSites[0], isCustom = false) => (
    <TouchableOpacity
      key={shortcut.id}
      style={[
        styles.shortcutCard,
      ...
      onLongPress={isCustom ? () => handleRemoveShortcut(shortcut.id) : undefined}
    >
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.subtitleText, { color: isDark ? '#8E8E93' : '#6B6B6B' }]}>
          智能浏览，AI助力
        </Text>
      </View>
      ...
      <View style={[styles.searchContainer, { backgroundColor: isDark ? '#2C2C2E' : '#FFFFFF' }]}>
        ...
        {searchQuery.length > 0 && (
          <TouchableOpacity onPress={() => setSearchQuery('')}>
            <Ionicons name="close-circle" size={20} color={isDark ? '#8E8E93' : '#6B6B6B'} />
          </TouchableOpacity>
        )}
      </View>
        ...
        >
          <View style={styles.aiChatContent}>
            ...
            <View style={styles.aiChatTextContainer}>
              ...
              <Text style={styles.aiChatSubtitle}>点击开始智能对话，获得个性化帮助</Text>
            </View>
